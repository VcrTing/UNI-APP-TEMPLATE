...
const expose = {
    ...
    __msg: async () => {
        const res: ReportDataTableMsgOrigin = await server_report_data.msg( 
            report_data_net_util.get_query_param(prp.schema) 
    ...
    // 下拉刷新数据
    __cache: async () => await server_report_data.cache( report_data_net_util.get_query_param(prp.schema)  ),
    // 初次获取数据
    __first: async () => {
        prp.schema.__tbo_dynamic_pager = prp.dynamic
        console.log('查询之前的 schema =', prp.schema)
        const res: ONE = await server_report_data.first( report_data_net_util.get_query_param(prp.schema) )
        if (is_nice_one(res)) {
            // 先做刷新 table 信息
